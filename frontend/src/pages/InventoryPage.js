...
export default function InventoryPage() {
  ...
  const [newForm, setNewForm]             = useState({ name:'', notes:'', model_id:'' });
  ...
  const handleCreate = async (e) => {
    ...
    if (res.id) {
      setShowNew(false);
      setNewForm({ name:'', notes:'', model_id:'' });
      await loadSessions();
      const all = await api.get('/inventory/sessions');
      setSessions(all);
      const created = all.find(s => s.id === res.id);
  ...
  const handleScan = useCallback(async (barcode) => {
    ...
    try {
      ...
      setScanLog(prev => [{
        barcode: trimmed, message: res.message,
  ...
  const handleCloseSession = async (markAll) => {
    const res = await api.post(`/inventory/sessions/${activeSession.id}/close`, { mark_missing: markAll });
    if (res.success) {
      setShowClose(false);
      await loadSessions();
      const detail = await api.get(`/inventory/sessions/${activeSession.id}`);
  ...
  const scopeLabel = (s) => {
    if (!s.model_id) return 'Full Inventory';
    const m = models.find(m => m.id === s.model_id);
    return m ? `Model: ${m.name}` : 'Specific Model';
  };
    ...
    <div className="page">
      ...
      {sessions.length === 0 ? (
        <div className="card"><div className="empty-state">
          <div className="icon">🔍</div><p>No inventory sessions yet.</p>
        ...
        <div className="card">
          <div className="table-wrap">
            <table>
              <thead><tr><th>Session Name</th><th>Scope</th><th>Status</th><th>Scanned</th><th>Date</th><th>Actions</th></tr></thead>
              <tbody>
                {sessions.map(s => (
                  <tr key={s.id}>
                    <td><div className="table-name">{s.name}</div>{s.notes && <div className="table-sub">{s.notes}</div>}</td>
                    <td>
                      <span style={{ fontSize:12, background:'var(--surface2)', borderRadius:6, padding:'2px 8px', color:'var(--text2)' }}>
                        {scopeLabel(s)}
                      </span>
                    </td>
                    <td><span className={`badge ${s.status==='open'?'badge-green':'badge-gray'}`}>{s.status==='open'?'🟢 Open':'✓ Closed'}</span></td>
                    <td style={{fontWeight:600}}>{s.scanned_count||0}</td>
                    <td style={{fontSize:12,color:'var(--text2)'}}>{new Date(s.created_at).toLocaleDateString()}</td>
                    <td><button className="btn btn-ghost btn-sm" onClick={() => openSession(s)}>
                      {s.status==='open'?'📷 Continue':'📋 Results'}
                    </button></td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="modal">
            ...
            <form onSubmit={handleCreate}>
              ...
              <div className="form-group">
                ...
                <select className="form-select" value={newForm.model_id} onChange={e=>setNewForm({...newForm,model_id:e.target.value})}>
                  <option value="">🏭 Full Inventory (all models)</option>
                  {models.map(m=><option key={m.id} value={m.id}>{m.name}{m.category_name?` — ${m.category_name}`:''} ({m.total_assets} assets)</option>)}
                </select>
                <p style={{fontSize:12,color:'var(--text2)',marginTop:6}}>
                  Choose a specific model to audit just those assets, or leave blank for a full inventory count.
                </p>
              </div>
              <div className="form-group">
  ...
  if (view === 'scan') return (
    <div className="page" style={{maxWidth:900,margin:'0 auto'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          ...
          <div>
            ...
            <div className="page-subtitle">
              {activeSession?.model_id ? `Auditing: ${scopeLabel(activeSession)}` : 'Full inventory count'}
            </div>
          </div>
        </div>
      ...
      <div className="card" style={{marginBottom:16,border:'2px solid var(--accent)'}}>
        <div style={{textAlign:'center',marginBottom:14}}>
          ...
          <div style={{fontSize:13,color:'var(--text2)',marginTop:3}}>
            Scan every item that is physically present
            {activeSession?.model_id && <span> — <strong>{scopeLabel(activeSession)}</strong> only</span>}
          </div>
        </div>
        <div style={{display:'flex',gap:8}}>
          <input ref={inputRef} className="form-input mono" style={{fontSize:18,textAlign:'center',letterSpacing:2}}
            value={input} onChange={e=>setInput(e.target.value)}
            onKeyDown={e=>e.key==='Enter'&&handleScan(input)}
            placeholder="Scan or type barcode..." disabled={processing} autoFocus />
      ...
      <div className="card" style={{marginBottom:16}}>
        ...
        <div className="scan-feed" style={{maxHeight:260}}>
          {scanLog.length===0
            ? <div style={{textAlign:'center',padding:24,color:'var(--text2)',fontSize:13}}>Scans will appear here...</div>
            : scanLog.map((e,i)=>(
              <div key={i} className={`scan-item ${e.type}`}>
                <span className="scan-icon">{e.type==='success'?'✅':e.type==='warning'?'⚠️':'❌'}</span>
                <div className="scan-msg">{e.message}</div>
                <span className="scan-time mono">{e.barcode}</span>
                <span className="scan-time">{e.time}</span>
              </div>
            ))}
        </div>
      </div>
              ...
              <tbody>
                {(sessionDetail?.scanned||[]).map(s=>(
                  <tr key={s.id}>
                    ...
                    <td><button className="btn btn-ghost btn-sm" onClick={()=>handleUndoScan(s.asset_id)}>↩ Undo</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setShowClose(false)}>
          <div className="modal">
            ...
            <p style={{color:'var(--text2)',marginBottom:20,fontSize:13}}>
              Scanned <strong>{scannedCount}</strong> items. What should happen to unscanned assets?
            </p>
    ...
    <div className="page">
      <div className="page-header">
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          ...
          <div>
            <div className="page-title">{activeSession?.name}</div>
            <div className="page-subtitle">
              {scopeLabel(activeSession)} · {activeSession?.status==='open'?'🟢 In progress':'✓ Closed'}
            </div>
          </div>
        </div>
      ...
      <div className="grid-3" style={{marginBottom:20}}>
        {[
          ...
          {label:'Not Scanned',      value:missing.length,   color:missing.length>0?'var(--red)':'var(--text2)'},
      ...
      <div className="card" style={{marginBottom:16}}>
        ...
        {scanned.length===0
          ? <div className="empty-state" style={{padding:20}}><p>No items scanned</p></div>
      ...
      {activeSession?.status==='closed' && (
        <div className="card" style={{border:missing.length>0?'1px solid rgba(239,68,68,.3)':undefined}}>
          ...
          {missing.length===0
            ? <p style={{color:'var(--text2)',fontSize:13}}>Every asset was scanned.</p>
            : <div className="table-wrap"><table>
